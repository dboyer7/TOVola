#ifndef TOVOLA_SOLVE_H
#define TOVOLA_SOLVE_H
...
/*********************************************************************************************************************************************************
//This is the header file that contains all the information about the TOVs and
integration schemes. Used in conjunction with GSL in the driver function.
*********************************************************************************************************************************************************/
...
/* Structure to hold raw TOV data. Only used by these functions, so is declared
 * in solve.h */
typedef struct {
  // EOS type
  ...
  // Additional declarations, to pass through ETK parameters in parfile without
...
static CCTK_INT TOVola_do_we_terminate(CCTK_REAL r, CCTK_REAL y[],
                                       TOVola_data_struct *TOVdata) {

  if (TOVdata->eos_type == 2) {
    const CCTK_REAL PMin = exp(
        TOVdata->ghl_eos->lp_of_lr[0]); // PMin is not zero on the table, so we
                                        // don't want to exceed table limits
    if (y[0] <= PMin) {
      return 1;
    }
  } else {
...
static void TOVola_evaluate_rho_and_eps(CCTK_REAL r, const CCTK_REAL y[],
                                        TOVola_data_struct *TOVdata) {
  ...
  if (TOVdata->eos_type == 0) {
    ...
    ghl_hybrid_compute_P_cold_and_eps_cold(TOVdata->ghl_eos, aRho_baryon,
                                           &aPress, &eps);
  ...
  // Tabulated EOS
    ...
    if (y[TOVOLA_PRESSURE] > PMin) { // Assure you are not exceeding table
                                     // bounds
      // Use GRHayL function to find our current rho_baryon and rho_energy on
      // the table.
      TOVdata->rho_baryon = ghl_tabulated_compute_rho_from_P(
          TOVdata->ghl_eos, y[TOVOLA_PRESSURE]);
      CCTK_REAL eps = ghl_tabulated_compute_eps_from_rho(TOVdata->ghl_eos,
                                                         TOVdata->rho_baryon);
...
static CCTK_INT TOVola_ODE(CCTK_REAL r_Schw, const CCTK_REAL y[],
                           CCTK_REAL dydr_Schw[], void *params) {
  ...
  // At the center of the star (r_Schw == 0), the TOV equations diverge, so we
  ...
  // TOV Equations
    ...
    dydr_Schw[TOVOLA_PRESSURE] =
        -((rho_energy + y[TOVOLA_PRESSURE]) *
          (mass_term + 8.0 * M_PI * r_Schw * r_Schw * y[TOVOLA_PRESSURE])) /
        (2.0 * r_Schw * denominator); // dP/dr
    dydr_Schw[TOVOLA_NU] =
        ((mass_term) + 8.0 * M_PI * r_Schw * r_Schw * y[TOVOLA_PRESSURE]) /
        (r_Schw * denominator); // dnu/dr
    dydr_Schw[TOVOLA_MASS] = 4.0 * M_PI * r_Schw * r_Schw * rho_energy; // dM/dr
    dydr_Schw[TOVOLA_R_ISO] =
        y[TOVOLA_R_ISO] /
        (r_Schw *
         sqrt(1.0 - mass_term)); // dr_iso/dr (r_iso = isotropic radius, also
                                 // known as rbar in literature)
  }
...
static CCTK_INT TOVola_jacobian_placeholder(CCTK_REAL t, const CCTK_REAL y[],
                                            CCTK_REAL *restrict dfdy,
                                            CCTK_REAL dfdt[], void *params) {
...
static void TOVola_get_initial_condition(CCTK_REAL y[],
                                         TOVola_data_struct *TOVdata) {
  ...
  if (TOVdata->eos_type == 0) {
    ...
    TOVdata->rho_energy = pow(y[TOVOLA_PRESSURE] / aK, 1.0 / aGamma) +
                          y[TOVOLA_PRESSURE] / (aGamma - 1.0);
  }
  ...
  // Piecewise Polytrope
    ...
    ghl_hybrid_get_K_and_Gamma(TOVdata->ghl_eos, rhoC_baryon, &aK, &aGamma);
    ghl_hybrid_compute_P_cold_and_eps_cold(TOVdata->ghl_eos, rhoC_baryon,
                                           &y[TOVOLA_PRESSURE], &eps);
    y[TOVOLA_PRESSURE] = aK * pow(rhoC_baryon, aGamma); // Pressure
    y[TOVOLA_NU] = 0.0;                                 // nu
    y[TOVOLA_MASS] = 0.0;                               // mass
  ...
  // Tabulated EOS
    ...
    CCTK_REAL rhoC_baryon = TOVdata->central_baryon_density;
    y[TOVOLA_PRESSURE] =
        ghl_tabulated_compute_P_from_rho(TOVdata->ghl_eos, rhoC_baryon);
    y[TOVOLA_NU] = 0.0;
    y[TOVOLA_MASS] = 0.0;
    y[TOVOLA_R_ISO] = 0.0;
    ...
    TOVdata->rho_baryon = rhoC_baryon;
    CCTK_REAL eps =
        ghl_tabulated_compute_eps_from_rho(TOVdata->ghl_eos, rhoC_baryon);
    TOVdata->rho_energy = (TOVdata->rho_baryon) * (1.0 + eps);
  }
}

/* Assign constants after each integration step */
static void TOVola_assign_constants(CCTK_REAL c[],
                                    TOVola_data_struct *TOVdata) {
...
static CCTK_INT setup_ode_system(const char *ode_method,
  ...
  if (CCTK_EQUALS(ode_method, "ARKF")) {
    *driver = gsl_odeiv2_driver_alloc_y_new(
        system, gsl_odeiv2_step_rkf45, TOVdata->initial_ode_step_size,
        TOVdata->error_limit, TOVdata->error_limit);
  } else if (CCTK_EQUALS(ode_method, "ADP8")) {
    *driver = gsl_odeiv2_driver_alloc_y_new(
        system, gsl_odeiv2_step_rk8pd, TOVdata->initial_ode_step_size,
        TOVdata->error_limit, TOVdata->error_limit);
  } else {
...
static CCTK_INT initialize_tovola_data(TOVola_data_struct *TOVdata) {
  TOVdata->rSchw_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numels_alloced_TOV_arr);
  TOVdata->rho_energy_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numels_alloced_TOV_arr);
  TOVdata->rho_baryon_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numels_alloced_TOV_arr);
  TOVdata->P_arr = (CCTK_REAL *restrict)malloc(sizeof(CCTK_REAL) *
                                               TOVdata->numels_alloced_TOV_arr);
  TOVdata->M_arr = (CCTK_REAL *restrict)malloc(sizeof(CCTK_REAL) *
                                               TOVdata->numels_alloced_TOV_arr);
  TOVdata->nu_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numels_alloced_TOV_arr);
  TOVdata->Iso_r_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numels_alloced_TOV_arr);
...
static CCTK_INT
initialize_ID_persist_data(TOVola_ID_persist_struct *TOVola_ID_persist,
                           TOVola_data_struct *TOVdata) {
  TOVola_ID_persist->r_Schw_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numpoints_actually_saved);
  TOVola_ID_persist->rho_energy_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numpoints_actually_saved);
  TOVola_ID_persist->rho_baryon_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numpoints_actually_saved);
  TOVola_ID_persist->P_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numpoints_actually_saved);
  TOVola_ID_persist->M_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numpoints_actually_saved);
  TOVola_ID_persist->expnu_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numpoints_actually_saved);
  TOVola_ID_persist->exp4phi_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numpoints_actually_saved);
  TOVola_ID_persist->r_iso_arr = (CCTK_REAL *restrict)malloc(
      sizeof(CCTK_REAL) * TOVdata->numpoints_actually_saved);

  if (!TOVola_ID_persist->r_Schw_arr || !TOVola_ID_persist->rho_energy_arr ||
    ...
    CCTK_ERROR(
        "Memory allocation failed for TOVola_ID_persist_struct arrays.\n");
    return -1;
  }
  return 0;
}
...
static void TOVola_Normalize_and_set_data_integrated(
    TOVola_data_struct *TOVdata, CCTK_REAL *restrict r_Schw,
  ...
  const CCTK_REAL R_Schw_surface =
      r_Schw[TOVdata->numpoints_actually_saved - 1];
  ...
  const CCTK_REAL normalize =
      0.5 *
      (sqrt(R_Schw_surface * (R_Schw_surface - 2.0 * M_surface)) +
       R_Schw_surface - M_surface) /
      r_iso_surface;

  /* Normalize r_iso and calculate expnu and exp4phi */
  for (CCTK_INT i = 0; i < TOVdata->numpoints_actually_saved; i++) {
    r_iso[i] *= normalize;
    expnu[i] = exp(expnu[i] - nu_surface +
                   log(1.0 - 2.0 * M_surface / R_Schw_surface));
    exp4phi[i] = (r_Schw[i] / r_iso[i]) * (r_Schw[i] / r_iso[i]);
  }
}

/* Extend data to r<0, to ensure we can interpolate to r=0 */
static void extend_to_negative_r(CCTK_REAL *restrict arr,
                                 const CCTK_REAL parity,
                                 CCTK_REAL *restrict tmp,
                                 const TOVola_data_struct *restrict TOVdata) {
  for (CCTK_INT i = 0; i < NEGATIVE_R_INTERP_BUFFER; i++)
