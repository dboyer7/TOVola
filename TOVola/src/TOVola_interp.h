...
static void TOVola_TOV_interpolate_1D(
  ...
  if (rr_iso < r_iso_max_inside_star) { // If we are INSIDE the star, we need to
    ...
    for (CCTK_INT i = idxmin; i < idxmin + Interpolation_Stencil; i++) {
      r_iso_sample[i - idxmin] = r_iso_arr[i];
    }
    CCTK_REAL l_i_of_r[Max_Interpolation_Stencil];
    for (CCTK_INT i = 0; i < Interpolation_Stencil; i++) {
      CCTK_REAL numer = 1.0;
      CCTK_REAL denom = 1.0;
      for (CCTK_INT j = 0; j < Interpolation_Stencil; j++) {
        if (j != i) {
